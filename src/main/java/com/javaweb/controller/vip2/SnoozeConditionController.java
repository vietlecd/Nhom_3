...
public class SnoozeConditionController {
    ...
    @PostMapping("/create")
    public ResponseEntity<?> createTriggerCondition(@RequestParam("SnoozeType") String triggerType, @RequestBody Map<String, Object> dtoMap) {
        try {
            switch (triggerType) {
                case "spot":
                    SnoozePriceDTO snoozeDTO = objectMapper.convertValue(dtoMap, SnoozePriceDTO.class);
                    spotPriceTriggerService.createSnooze(snoozeDTO);
                    break;
                case "future":
